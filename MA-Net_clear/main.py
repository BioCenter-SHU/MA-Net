...
def train_epoch(k, model, train_dataloader, epoch, criterion, optimizer, writer):
    ...
    writer.add_scalar('train_recall_epoch', total_recall, epoch)
    writer.add_scalar('train_precision_epoch', total_precision, epoch)
    writer.add_scalar('train_auc_epoch', total_auc, epoch)
    writer.add_scalar('train_acc_epoch', total_acc, epoch)
    writer.add_scalar('train_f1_epoch', total_f1, epoch)
    ...
    with torch.no_grad():
        for step, inputs in enumerate(val_dataloader):
            ...
            if (step + 1) % 10 == 0:
                ...
                print(print_string)
        print_string = ('Testing Results: loss {loss.avg:.5f}, Top-1 {top1.avg:.3f}, Top-2 {top5.avg:.3f}'
...
def k_fold(k,image_dir,num_epochs,batch_size, clip_len,  learning_rate, lr_steps, frame_path, num_workers):
    ...
    for i in range(k): 
        ...
        for epoch in trange(num_epochs):
            ...
            train_epoch(i, model, train_loader, epoch, criterion, optimizer, writer)

...
class Action(nn.Module):
    def __init__(self, net, n_segment=3, shift_div=8):
        ...
        # keyframe attention
        self.kf_conv1 = nn.Conv2d(self.in_channels, self.reduced_channels, kernel_size=(1, 1), stride=(1, 1), bias=False, padding=(0, 0))
        ...
        print('====USE keyframe 3 CS method 5 Block====')
    ...
    def forward(self, x):
        ...
        x_kf = self.kf_conv1(x)
        x_kf = x_kf.view(n_batch, self.n_segment, self.reduced_channels, h, w)
        x_kf = self.global_max_pool(x_kf).view(n_batch, -1)   # n, t
        # x_kf = self.keyframe_fc1(x_kf)  # n, t//2
        # x_kf = self.kf_relu(x_kf)
        # x_kf = self.keyframe_fc2(x_kf)  # n, t
        x_kf_attention = self.keyframe_softmax(x_kf).view(nt, 1, 1, 1)  # nt, 1, 1, 1
        ...
        x_p1 = x_p2 * x_p1 + x_p2 + x  # 等价于x_p1 = x_p2 * x_p1, CBAM_out = x_p1 + x_p2  # nt, c, h, c
        
        x_p1_kf = x_kf_attention * x_p1 + x

        out = self.net(x_kf+ x_p1 + x_p1_kf + x)

...
class Action(nn.Module):
    ...
    def forward(self, x):
        ...
        x_kf = self.relu(x_kf)
        # x_kf = self.leackrelu(x_kf)     # 
        x_kf = self.keyframe_fc2(x_kf)  # n, t, 1, 1, 1
        x_kf = self.keyframe_softmax(x_kf)  # 
        ...
        CBAM_out = CBAM_out.view(n_batch, self.n_segment, c, h, w)
        x_kf = x_kf * CBAM_out
        CBAM_out = CBAM_out.view(-1, c, h, w)         # nt, c, h, w
        x_kf = x_kf.view(-1, c, h, w)   # nt, c, h, w
        kf_cbam = x_kf + CBAM_out
        out = self.net(kf_cbam + x)

...
class Action(nn.Module):
    def __init__(self, net, n_segment=3, shift_div=8):
        ...
        # keyframe attention
        # self.keyframe_conv1 = nn.Conv3d(self.in_channels, self.out_channels, 1, kernel_size=(3, 3, 3), stride=(1, 1 ,1), bias=False, padding=(1, 1, 1))
        self.global_max_pool = nn.AdaptiveMaxPool3d(1)
        self.keyframe_fc1 = nn.Conv3d(self.n_segment, self.n_segment // 2, kernel_size=(1, 1, 1), stride=(1 ,1, 1), bias=False, padding=(0, 0, 0))
        # self.keyframe_fc1 = nn.Linear(self.n_segment, self.n_segment // 2, bias=False)
        self.relu = nn.ReLU(inplace=True)
        self.keyframe_fc2 = nn.Conv3d(self.n_segment // 2, self.n_segment, kernel_size=(1, 1, 1), stride=(1 ,1, 1), bias=False, padding=(0, 0, 0))
        # self.keyframe_fc2 = nn.Linear(self.n_segment // 2, self.n_segment, bias=False)
        self.keyframe_softmax = nn.Softmax(dim=1)

        print('====USE keyframe Block====')
    ...
    def forward(self, x):
        ...
        # ==================keyframe====================
        # x_kf = self.keyframe_conv1(x)   # nt, c, h, w
        x_kf = x.view(n_batch, self.n_segment, c, h, w)
        x_kf = self.global_max_pool(x_kf)   # n, t, 1, 1, 1
        x_kf = self.keyframe_fc1(x_kf)  # n, t//2, 1, 1, 1
        x_kf = self.relu(x_kf)
        x_kf = self.keyframe_fc2(x_kf)  # n, t, 1, 1, 1
        x_kf = self.keyframe_softmax(x_kf)  # n, t, 1, 1, 1
        x = x.view(n_batch, self.n_segment, c, h, w)
        x_kf = x_kf * x 
        x = x.view(-1, c, h, w)         # nt, c, h, w
        x_kf = x_kf.view(-1, c, h, w)   # nt, c, h, w
        out = self.net(x_kf + x)
        ...
        # CBAM_out = x_p1 + x_p2
        # out = self.net(CBAM_out + x)

        # ====== 策略1：关键帧和CBAM 拼接 ======
        # x = x.view(n_batch, self.n_segment, c, h, w)
        # x_kf = x_kf_attention * x + x
        # x = x.view(-1, c, h, w)         # nt, c, h, w
        # kf_out = x_kf.view(-1, c, h, w)   # nt, c, h, w
        # kf_cbam = torch.cat([kf_out, CBAM_out], dim=1)    # nt, 2c, h, w
        # kf_cbam = self.cat(kf_cbam)     # nt, c, h, w
        # out = self.net(kf_cbam + x)
        ...
        # x = x.view(-1, c, h, w)         # nt, c, h, w
        # kf_out = x_kf.view(-1, c, h, w)   # nt, c, h, w
        # kf_cbam = kf_out + CBAM_out
        # out = self.net(kf_cbam + x)


        # ====== 策略3：关键帧修正，attention 和 CBAMout相乘 ====
        # x_kf_attention = x_kf_attention.view(-1, c, h, w)   # nt, c, h, w
        # kf_attention_cbam = CBAM_out * x_kf_attention + CBAM_out
        # out = self.net(kf_attention_cbam + x)


        # ====== 策略4：要从CBAM模块入手改, 从CBAM开始全部注释掉，修改CBAM =====
        # x = x.view(n_batch, self.n_segment, c, h, w)
        # x_kf = x_kf_attention * x + x
        # x = x.view(-1, c, h, w)         # nt, c, h, w
        # x_kf = x_kf.view(-1, c, h, w)   # nt, c, h, w
        # x_p2_1 = self.avg_pool(x_kf)
        # x_p2_1 = self.action_p2_squeeze(x_p2_1)
        # nt, c, h, w = x_p2_1.size()
        # x_p2_1 = x_p2_1.view(n_batch, self.n_segment, c, 1, 1).squeeze(-1).squeeze(-1).transpose(2,1).contiguous()
        # x_p2_1 = self.action_p2_conv1(x_p2_1)

...
class Action(nn.Module):
    def __init__(self, net, n_segment=3, shift_div=8):
        ...
        self.cat = nn.Conv2d(2*self.in_channels, self.in_channels, kernel_size=(3, 3), 
                                    stride=(1 ,1), bias=False, padding=(1, 1)) 

        print('====USE TC_TS_sum Block====')
    ...
    def forward(self, x):
        ...
        x_p1_avg = x.mean(1, keepdim=True)   # nt, 1, h, w
        x_p1_max, _ = x.max(1, keepdim=True) # nt, 1, h, w
        ...
        x_p1 = torch.cat([x_p1_avg, x_p1_max], dim=1)   # n_batch, 2, c, h, w
        ...
        out = self.net(self.cat(torch.cat([x_p1, x_p2], dim=1)))

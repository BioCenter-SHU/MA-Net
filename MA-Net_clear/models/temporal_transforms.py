...
class TemporalUniformCrop_train(object):
    ...
    def __call__(self, frame_indices):
        ...
        if average_duration > 0:
            ...
            # out = np.multiply(list(range(self.size)), average_duration) + randint(average_duration, size=self.size)
            # 固定选取第一帧
            out = np.multiply(list(range(self.size)), average_duration) 
            # 固定选取第二帧
            # out = np.multiply(list(range(self.size)), average_duration) + 1
            # 选末尾的帧
            # [0, 1, 2, ..., 29] * 4 = [0, 4, 8, ..., 116]
            # out = np.multiply(list(range(self.size)), average_duration) + average_duration - 1
        else:                 
